...
const BookFormModal = ({ open, handleClose, onSuccess, editingBook = null }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (editingBook) {
      setFormData({
        ...
        endereco: editingBook.endereco || '',
        contato: editingBook.contato || '',
        imagem: editingBook.imagem || null
      });
      setImagePreview(editingBook.imagem);
    } else {
      setFormData({
        titulo: '',
        autor: '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = editingBook 
        ? `http://localhost:3000/api/livros/${editingBook.id}`
      ...
      if (!response.ok) {
        throw new Error(editingBook ? 'Erro ao atualizar livro' : 'Erro ao criar livro');
      }
      ...
      setLoading(false);
      alert(editingBook 
        ? 'Erro ao atualizar livro. Por favor, tente novamente.' 
        : 'Erro ao criar livro. Por favor, tente novamente.'
      );
    }
  };
    ...
    <StyledDialog open={open} onClose={handleClose}>
      ...
      <DialogActions>
        ...
        <Button
          ...
          color='green'
        >
          {loading ? <CircularProgress size={24} color="inherit" /> : (editingBook ? 'Salvar Alterações' : 'Cadastrar Livro')}
        </Button>
      </DialogActions>
    </StyledDialog>
  );
